...
import { howBeforeMutate } from "../src/agent/gates.ts";
...
import { isMessage } from "../src/provider/types.ts";
...
describe("regressions", () => {
  ...
  test("R13 reminder is developer, not a fake user turn", async () => {
    const dir = tmpProject();
    const rt = await bootFake(dir, [fakeTurn([assistant("ok")])]);
    await run(rt, "hi");
    const calls = (rt.client as FakeClient).calls;
    expect(calls.length).toBeGreaterThan(0);
    const reminders = calls[0]!.input.filter(isMessage).filter((m) => m.meta?.kind === "reminder");
    expect(reminders).toHaveLength(1);
    expect(reminders[0]!.role).toBe("developer");
    const users = calls[0]!.input.filter(isMessage).filter((m) => m.role === "user");

...
5. **reminder** — plan, durable state, remaining budget, sent as `role: developer` so Grok does not treat it as the user talking. Appended only to this request's input. Not persisted. If you put this in `instructions`, every turn busts the cache.
...
8. **checkpoint** — if a tool reported `changed`, snapshot the shadow git and refresh the map.
